...
export const projects: Project[] = [
  {
    ...
    description:
      'A full-stack case management system for Manitoba traffic defense law firms — tracking HTA violation cases (speeding, careless driving, red light infractions, and more) from intake through resolution: Open → In Progress → Won / Lost / Dismissed, with client profiles and a per-case document library.\n\nThe core feature is a human-in-the-loop AI intake agent built with LangGraph: when a new case arrives, a multi-step agentic workflow runs — extracting document details, looking up real HTA statutes, retrieving similar past cases from the database, and drafting a legal memo — then pauses for lawyer review and approval before anything is written to the database. The AI never acts unilaterally on legal work.\n\nAgent state is persisted with LangGraph\'s PostgresSaver on AWS RDS, so workflows survive server restarts and resume across requests. LLM outputs are grounded in real statute lookups and live database queries (tool use) to prevent hallucinated legal citations — critical in a regulated domain.',
    tags: ['LangGraph', 'Human-in-the-loop', 'Next.js', 'TypeScript', 'FastAPI', 'PostgreSQL', 'AWS', 'Docker', 'Claude API'],
