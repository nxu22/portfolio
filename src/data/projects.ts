...
export const projects: Project[] = [
  {
    ...
    description:
      'A case management system built for Manitoba traffic defense law firms — tracking HTA violation cases (speeding, careless driving, red light infractions, and more) from intake through resolution: Open → In Progress → Won / Lost / Dismissed. Lawyers get a real-time dashboard of all active cases, client profiles with driver\'s license records, and a full document library per case.\n\nThe standout feature is one-click AI document analysis: upload a traffic ticket, court notice, or evidence file directly to a private AWS S3 bucket, then hit a button to get an instant Claude-generated summary that extracts the offence, key dates, fines, and flags defense considerations. Documents are always accessed through presigned URLs — files are never publicly exposed.',
